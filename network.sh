...
function createChannel12() {
  ...
  # now run the script that creates a channel. This script uses configtxgen once
  # to create the channel creation transaction and the anchor peer updates.
  scripts/createChannel12.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}

function createChannel23() {
  # Bring up the network if it is not already up.
  ...
  # now run the script that creates a channel. This script uses configtxgen once
  # to create the channel creation transaction and the anchor peer updates.
  scripts/createChannel23.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}
...
function deployCC23() {
  scripts/deployCC23.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
elif [ "$MODE" == "createChannel12" ]; then
  infoln "Creating channel12 '${CHANNEL_NAME}'."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
elif [ "$MODE" == "createChannel23" ]; then
  infoln "Creating channel23 '${CHANNEL_NAME}'."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
elif [ "$MODE" == "down" ]; then
