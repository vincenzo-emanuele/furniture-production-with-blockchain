...
installChaincode() {
  ...
  res=$?
  { set +x; } 2>/dev/null
# Prendo l'output e se è stato già installato con successo continuo senza uscire
  LOG=$(cat log.txt)
  echo $LOG
  if [[ ! $LOG == *"chaincode already successfully installed"* ]]; then
  	verifyResult $res "Chaincode installation on peer0.org${ORG} has failed"
  	successln "Chaincode is installed on peer0.org${ORG}"
  else
  	successln "Il Chaincode e' stato già installato, continuo..."
  fi
}
...
checkCommitReadiness 3 "\"Org2MSP\": true" "\"Org3MSP\": false"
